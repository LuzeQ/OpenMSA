 ...
 * - interactive without interactiveConfig or widget config → inject default interactiveConfig
...
export function applyOutlineFallbacks(
  ...
  const hasWidgetConfig = Boolean(outline.widgetType && outline.widgetOutline);

  if (outline.type === 'interactive' && !outline.interactiveConfig && !hasWidgetConfig) {
    log.warn(
      `Interactive outline "${outline.title}" missing interactiveConfig and widget config, injecting default interactiveConfig`,
    );
