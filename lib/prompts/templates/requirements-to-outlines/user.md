...
4. **PBL scenes**: If the requirement asks for project-based learning, real-world tasks, role collaboration, integrated practice, engineering challenges, research tasks, or multi-step projects, include exactly one `"type": "pbl"` scene when the scope supports it. Its `pblConfig` must be complete, and you may add `language` if that helps keep downstream PBL generation aligned.
...
7. **Language**: Infer from the user's requirement text and context, then output all content in the inferred language. If you include `pblConfig.language`, keep it consistent with the inferred course language.
