...
- If the requirement explicitly mentions project-based learning, real-world tasks, role collaboration, integrated practice, engineering challenges, research tasks, or multi-step projects, you SHOULD include exactly 1 PBL scene unless the course scope is clearly too small for it
- PBL scenes **require** a `pblConfig` object with: projectTopic, projectDescription, targetSkills, issueCount. You MAY include `language` when it helps downstream tools stay aligned with the course language
...
| pblConfig         | object                   | ❌       | Required for pbl type, contains projectTopic/projectDescription/targetSkills/issueCount, and may optionally include language |
...
7. `pbl` scenes must include `pblConfig` with `projectTopic`, `projectDescription`, `targetSkills`, `issueCount`. Include `language` when needed to keep downstream PBL generation aligned.
...
9. **Language**: Infer from the user's requirement text and context. Output all scene content in the inferred language, and keep any optional `pblConfig.language` consistent with that directive.
